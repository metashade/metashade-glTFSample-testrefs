...
float getSpotShadow(Light light, vec3 Pw)
{
    ...
    vec3 _584 = p4Shadow.xyz / vec3(p4Shadow.w);
    p4Shadow = vec4(_584.x, _584.y, _584.z, p4Shadow.w);
...
PsOut src_main(VsOut psIn)
{
    ...
    PbrParams param_var_pbrParams = pbrParams;
    vec3 _188 = applySpotLight(param_var_light, param_var_Nw, param_var_Vw, param_var_Pw, param_var_pbrParams);
    psOut.rgbaColor = vec4(_188.x, _188.y, _188.z, psOut.rgbaColor.w);
    PbrParams param_var_pbrParams_1 = pbrParams;
    vec3 param_var_N = Nw;
    vec3 param_var_V = Vw;
    vec3 _205 = psOut.rgbaColor.xyz + (getIbl(param_var_pbrParams_1, param_var_N, param_var_V) * cbPerFrame.g_fIblFactor);
    psOut.rgbaColor = vec4(_205.x, _205.y, _205.z, psOut.rgbaColor.w);
    vec4 occlusionSample = texture(sampler2D(g_tOcclusion, g_sOcclusion), psIn.uv0, cbPerFrame.g_lodBias);
    vec3 _224 = psOut.rgbaColor.xyz * occlusionSample.x;
    psOut.rgbaColor = vec4(_224.x, _224.y, _224.z, psOut.rgbaColor.w);
    ...
    vec3 _253 = psOut.rgbaColor.xyz + rgbEmissive;
    psOut.rgbaColor = vec4(_253.x, _253.y, _253.z, psOut.rgbaColor.w);
    return psOut;
}
