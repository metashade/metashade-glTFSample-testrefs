...
vec3 getNormal(VsOut psIn)
{
    ...
    vec3 PwDx = dFdx(psIn.Pw);
    vec3 PwDy = dFdy(psIn.Pw);
    vec2 uvDx = dFdx(psIn.uv0);
    vec2 uvDy = dFdy(psIn.uv0);
    vec3 Tw = ((PwDx * uvDy.y) - (PwDy * uvDx.y)) / vec3((uvDx.x * uvDy.y) - (uvDy.x * uvDx.y));
...
float getSpotShadow(Light light, vec3 Pw)
{
    ...
    vec3 _587 = p4Shadow.xyz / vec3(p4Shadow.w);
    p4Shadow = vec4(_587.x, _587.y, _587.z, p4Shadow.w);
...
PsOut src_main(VsOut psIn)
{
    ...
    PbrParams param_var_pbrParams = pbrParams;
    vec3 _191 = applySpotLight(param_var_light, param_var_Nw, param_var_Vw, param_var_Pw, param_var_pbrParams);
    psOut.rgbaColor = vec4(_191.x, _191.y, _191.z, psOut.rgbaColor.w);
    PbrParams param_var_pbrParams_1 = pbrParams;
    vec3 param_var_N = Nw;
    vec3 param_var_V = Vw;
    vec3 _208 = psOut.rgbaColor.xyz + (getIbl(param_var_pbrParams_1, param_var_N, param_var_V) * cbPerFrame.g_fIblFactor);
    psOut.rgbaColor = vec4(_208.x, _208.y, _208.z, psOut.rgbaColor.w);
    vec4 occlusionSample = texture(sampler2D(g_tOcclusion, g_sOcclusion), psIn.uv0, cbPerFrame.g_lodBias);
    vec3 _227 = psOut.rgbaColor.xyz * occlusionSample.x;
    psOut.rgbaColor = vec4(_227.x, _227.y, _227.z, psOut.rgbaColor.w);
    ...
    vec3 _256 = psOut.rgbaColor.xyz + rgbEmissive;
    psOut.rgbaColor = vec4(_256.x, _256.y, _256.z, psOut.rgbaColor.w);
    return psOut;
}
