...
float getSpotShadow(Light light, vec3 Pw)
{
    ...
    vec3 _473 = p4Shadow.xyz / vec3(p4Shadow.w);
    p4Shadow = vec4(_473.x, _473.y, _473.z, p4Shadow.w);
...
PsOut src_main(VsOut psIn)
{
    ...
    PbrParams param_var_pbrParams = pbrParams;
    vec3 _182 = applySpotLight(param_var_light, param_var_Nw, param_var_Vw, param_var_Pw, param_var_pbrParams);
    psOut.rgbaColor = vec4(_182.x, _182.y, _182.z, psOut.rgbaColor.w);
    PbrParams param_var_pbrParams_1 = pbrParams;
    vec3 param_var_N = Nw;
    vec3 param_var_V = Vw;
    vec3 _199 = psOut.rgbaColor.xyz + (getIbl(param_var_pbrParams_1, param_var_N, param_var_V) * cbPerFrame.g_fIblFactor);
    psOut.rgbaColor = vec4(_199.x, _199.y, _199.z, psOut.rgbaColor.w);
    vec3 rgbEmissive = cbPerObject.g_perObjectPbrFactors.rgbaEmissive.xyz * cbPerFrame.g_fPerFrameEmissiveFactor;
    vec3 _216 = psOut.rgbaColor.xyz + rgbEmissive;
    psOut.rgbaColor = vec4(_216.x, _216.y, _216.z, psOut.rgbaColor.w);
    return psOut;
}
