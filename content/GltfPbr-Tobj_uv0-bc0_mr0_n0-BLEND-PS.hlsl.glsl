...
float getSpotShadow(Light light, vec3 Pw)
{
    ...
    vec3 _511 = p4Shadow.xyz / vec3(p4Shadow.w);
    p4Shadow = vec4(_511.x, _511.y, _511.z, p4Shadow.w);
...
PsOut src_main(VsOut psIn)
{
    ...
    PbrParams _156 = metallicRoughness(param_var_psIn_1);
    PbrParams pbrParams = _156;
    PsOut psOut;
    ...
    PbrParams param_var_pbrParams = pbrParams;
    vec3 _186 = applySpotLight(param_var_light, param_var_Nw, param_var_Vw, param_var_Pw, param_var_pbrParams);
    psOut.rgbaColor = vec4(_186.x, _186.y, _186.z, psOut.rgbaColor.w);
    PbrParams param_var_pbrParams_1 = pbrParams;
    vec3 param_var_N = Nw;
    vec3 param_var_V = Vw;
    vec3 _203 = psOut.rgbaColor.xyz + (getIbl(param_var_pbrParams_1, param_var_N, param_var_V) * cbPerFrame.g_fIblFactor);
    psOut.rgbaColor = vec4(_203.x, _203.y, _203.z, psOut.rgbaColor.w);
    vec3 rgbEmissive = cbPerObject.g_perObjectPbrFactors.rgbaEmissive.xyz * cbPerFrame.g_fPerFrameEmissiveFactor;
    vec3 _220 = psOut.rgbaColor.xyz + rgbEmissive;
    psOut.rgbaColor = vec4(_220.x, _220.y, _220.z, psOut.rgbaColor.w);
    return psOut;
}
...
void main()
{
    ...
    PsOut _118 = src_main(param_var_psIn);
    out_var_SV_TARGET = _118.rgbaColor;
}
