...
float3 getNormal(VsOut psIn)
{
	...
	float3 PwDx = ddx(psIn.Pw);
	float3 PwDy = ddy(psIn.Pw);
	float2 uvDx = ddx(psIn.uv0);
	float2 uvDy = ddy(psIn.uv0);
	float3 Tw = ((uvDy.y * PwDx) - (uvDx.y * PwDy)) / ((uvDx.x * uvDy.y) - (uvDy.x * uvDx.y));
...
PsOut main(VsOut psIn)
{
	...
	rgbEmissive *= emissiveSample.rgb;
