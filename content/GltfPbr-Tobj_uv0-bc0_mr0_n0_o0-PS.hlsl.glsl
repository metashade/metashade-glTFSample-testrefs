...
layout(set = 0, binding = 3) uniform texture2D g_tOcclusion;
layout(set = 0, binding = 3) uniform sampler g_sOcclusion;
layout(set = 0, binding = 4) uniform texture2D g_tIblBrdfLut;
layout(set = 0, binding = 4) uniform sampler g_sIblBrdfLut;
layout(set = 0, binding = 5) uniform textureCube g_tIblDiffuse;
layout(set = 0, binding = 5) uniform sampler g_sIblDiffuse;
layout(set = 0, binding = 6) uniform textureCube g_tIblSpecular;
layout(set = 0, binding = 6) uniform sampler g_sIblSpecular;
...
float getSpotShadow(Light light, vec3 Pw)
{
    ...
    vec3 _528 = p4Shadow.xyz / vec3(p4Shadow.w);
    p4Shadow = vec4(_528.x, _528.y, _528.z, p4Shadow.w);
...
PsOut src_main(VsOut psIn)
{
    ...
    PbrParams pbrParams = metallicRoughness(param_var_psIn_1);
    PsOut psOut;
    ...
    PbrParams param_var_pbrParams = pbrParams;
    vec3 _192 = applySpotLight(param_var_light, param_var_Nw, param_var_Vw, param_var_Pw, param_var_pbrParams);
    psOut.rgbaColor = vec4(_192.x, _192.y, _192.z, psOut.rgbaColor.w);
    PbrParams param_var_pbrParams_1 = pbrParams;
    vec3 param_var_N = Nw;
    vec3 param_var_V = Vw;
    vec3 _209 = psOut.rgbaColor.xyz + (getIbl(param_var_pbrParams_1, param_var_N, param_var_V) * cbPerFrame.g_fIblFactor);
    psOut.rgbaColor = vec4(_209.x, _209.y, _209.z, psOut.rgbaColor.w);
    vec4 occlusionSample = texture(sampler2D(g_tOcclusion, g_sOcclusion), psIn.uv0, cbPerFrame.g_lodBias);
    vec3 _228 = psOut.rgbaColor.xyz * occlusionSample.x;
    psOut.rgbaColor = vec4(_228.x, _228.y, _228.z, psOut.rgbaColor.w);
    vec3 rgbEmissive = cbPerObject.g_perObjectPbrFactors.rgbaEmissive.xyz * cbPerFrame.g_fPerFrameEmissiveFactor;
    vec3 _245 = psOut.rgbaColor.xyz + rgbEmissive;
    psOut.rgbaColor = vec4(_245.x, _245.y, _245.z, psOut.rgbaColor.w);
    return psOut;
}
...
void main()
{
    ...
    out_var_SV_TARGET = src_main(param_var_psIn).rgbaColor;
}
