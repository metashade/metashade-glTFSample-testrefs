...
layout(set = 0, binding = 1) uniform texture2D g_tIblBrdfLut;
layout(set = 0, binding = 1) uniform sampler g_sIblBrdfLut;
layout(set = 0, binding = 2) uniform textureCube g_tIblDiffuse;
layout(set = 0, binding = 2) uniform sampler g_sIblDiffuse;
layout(set = 0, binding = 3) uniform textureCube g_tIblSpecular;
layout(set = 0, binding = 3) uniform sampler g_sIblSpecular;
...
float getSpotShadow(Light light, vec3 Pw)
{
    ...
    vec3 _451 = p4Shadow.xyz / vec3(p4Shadow.w);
    p4Shadow = vec4(_451.x, _451.y, _451.z, p4Shadow.w);
...
PsOut src_main(VsOut psIn)
{
    ...
    PbrParams param_var_pbrParams = pbrParams;
    vec3 _177 = applySpotLight(param_var_light, param_var_Nw, param_var_Vw, param_var_Pw, param_var_pbrParams);
    psOut.rgbaColor = vec4(_177.x, _177.y, _177.z, psOut.rgbaColor.w);
    PbrParams param_var_pbrParams_1 = pbrParams;
    vec3 param_var_N = Nw;
    vec3 param_var_V = Vw;
    vec3 _194 = psOut.rgbaColor.xyz + (getIbl(param_var_pbrParams_1, param_var_N, param_var_V) * cbPerFrame.g_fIblFactor);
    psOut.rgbaColor = vec4(_194.x, _194.y, _194.z, psOut.rgbaColor.w);
    vec3 rgbEmissive = cbPerObject.g_perObjectPbrFactors.rgbaEmissive.xyz * cbPerFrame.g_fPerFrameEmissiveFactor;
    vec3 _211 = psOut.rgbaColor.xyz + rgbEmissive;
    psOut.rgbaColor = vec4(_211.x, _211.y, _211.z, psOut.rgbaColor.w);
    return psOut;
}

void main()
{
    VsOut param_var_psIn = VsOut(gl_FragCoord, in_var_TEXCOORD0, in_var_TEXCOORD1, in_var_TEXCOORD2);
