...
float getSpotShadow(Light light, vec3 Pw)
{
    ...
    vec3 _454 = p4Shadow.xyz / vec3(p4Shadow.w);
    p4Shadow = vec4(_454.x, _454.y, _454.z, p4Shadow.w);
...
PsOut src_main(VsOut psIn)
{
    ...
    PbrParams param_var_pbrParams = pbrParams;
    vec3 _180 = applySpotLight(param_var_light, param_var_Nw, param_var_Vw, param_var_Pw, param_var_pbrParams);
    psOut.rgbaColor = vec4(_180.x, _180.y, _180.z, psOut.rgbaColor.w);
    PbrParams param_var_pbrParams_1 = pbrParams;
    vec3 param_var_N = Nw;
    vec3 param_var_V = Vw;
    vec3 _197 = psOut.rgbaColor.xyz + (getIbl(param_var_pbrParams_1, param_var_N, param_var_V) * cbPerFrame.g_fIblFactor);
    psOut.rgbaColor = vec4(_197.x, _197.y, _197.z, psOut.rgbaColor.w);
    vec3 rgbEmissive = cbPerObject.g_perObjectPbrFactors.rgbaEmissive.xyz * cbPerFrame.g_fPerFrameEmissiveFactor;
    vec3 _214 = psOut.rgbaColor.xyz + rgbEmissive;
    psOut.rgbaColor = vec4(_214.x, _214.y, _214.z, psOut.rgbaColor.w);
    return psOut;
}
