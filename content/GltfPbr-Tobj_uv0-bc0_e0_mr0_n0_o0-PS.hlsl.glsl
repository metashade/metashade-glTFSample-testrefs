...
float getSpotShadow(Light light, vec3 Pw)
{
    ...
    vec3 _543 = p4Shadow.xyz / vec3(p4Shadow.w);
    p4Shadow = vec4(_543.x, _543.y, _543.z, p4Shadow.w);
...
PsOut src_main(VsOut psIn)
{
    ...
    PbrParams param_var_pbrParams = pbrParams;
    vec3 _195 = applySpotLight(param_var_light, param_var_Nw, param_var_Vw, param_var_Pw, param_var_pbrParams);
    psOut.rgbaColor = vec4(_195.x, _195.y, _195.z, psOut.rgbaColor.w);
    PbrParams param_var_pbrParams_1 = pbrParams;
    vec3 param_var_N = Nw;
    vec3 param_var_V = Vw;
    vec3 _212 = psOut.rgbaColor.xyz + (getIbl(param_var_pbrParams_1, param_var_N, param_var_V) * cbPerFrame.g_fIblFactor);
    psOut.rgbaColor = vec4(_212.x, _212.y, _212.z, psOut.rgbaColor.w);
    vec4 occlusionSample = texture(sampler2D(g_tOcclusion, g_sOcclusion), psIn.uv0, cbPerFrame.g_lodBias);
    vec3 _231 = psOut.rgbaColor.xyz * occlusionSample.x;
    psOut.rgbaColor = vec4(_231.x, _231.y, _231.z, psOut.rgbaColor.w);
    ...
    vec3 _260 = psOut.rgbaColor.xyz + rgbEmissive;
    psOut.rgbaColor = vec4(_260.x, _260.y, _260.z, psOut.rgbaColor.w);
    return psOut;
}
